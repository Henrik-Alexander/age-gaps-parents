...
It is very likely that some of the change in the age distribution is driven by the impactful reunification, which caused migration as well as fertility postponement. Thus, we estimated non-parametric age-specific first birth rates seperately by birth region. The sample was split into persons who were born in East-Germany and respondents who were born in West Germany. Following common practice, respondents from Berlin were classified as East-German.
...
Once we have prepared the data, we estimate a discrete time survival regression with knots in 5-year intervals, with interactions between cohort and birth region. We than plot the predicted probabilities from the model in \@ref(fig:pred-reg)
